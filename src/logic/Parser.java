...
Stack<Integer> stackOfLines = new Stack<Integer>(); // Stack para guardar los n煤meros de l铆nea (para anidamiento de for/if)
private int numLineaDecl; 							// Guarda el n煤mero de l铆neal del comienzo de declaraci贸n
private int countSize; 								// Cuenta cantidad de elementos inicializados en el arreglo
private String elems; 								// Guarda los elementos iniciales de la colecci贸n
private String tipovar; 							// Almacena el tipo de la variable/colecci贸n para su seteo posterior en TS
private String expectedVarType; 					// Para control de tipos en la declaraci贸n
private ST_Control nodoRaiz; 						// Raiz del 谩rbol sint谩ctico
...
{ 
				...
				if (tipovar != expectedVarType){
					 msgStack.addMsg("Error: uso de constante de tipo distinto al declarado en l铆nea: " + lex.getNewLineCounter());
					 hasError = true;
				}
				expectedVarType = "";
			  }
...
>>>>>>> d8fb1c1... arreglado la extracci贸n de valor del indice dentro del foreach
{
					if(!symbolTable.get(val_peek(0).sval).isDeclared()){
						...
						msgStack.addMsg("Error: variable "+ val_peek(0).sval+" re-declarada en el n煤mero de l铆nea " + lex.getNewLineCounter());
						hasError = true;
					}
				}
break;
...
//#line 79 "gramatica.y"
{
				if(!symbolTable.get(val_peek(3).sval).isDeclared()) {
					...
					msgStack.addMsg("Error: colecci贸n "+ val_peek(3).sval+" re-declarada en el n煤mero de l铆nea " + lex.getNewLineCounter());
					hasError = true;
				}
			}
break;
...
{ yyval = val_peek(2); semanticStructStack.addMsg("L铆nea "+ stackOfLines.peek().intValue() +": Bloque de ejecuci贸n"); }
break;
case 24:
...
//#line 201 "gramatica.y"
{
			...
			if (((SyntacticTree)val_peek(2)).getType() != ((SyntacticTree)val_peek(0)).getType()){
				hasError = true;
				msgStack.addMsg("Error: Incompatibilidad de tipos entre las variables utilizadas en l铆nea: " + lex.getNewLineCounter());
				nodoCond = new ST_Common("ERROR", null, null);
			} else
...
{ 
		...
		if(!(symbolTable.containsKey(val_peek(2).sval) && symbolTable.get(val_peek(1).sval).isDeclared())) {
			msgStack.addMsg("Error: Colecci贸n  " + val_peek(1).sval + " no declarada.");
			hasError = true;
		}
		if (hasError) /*Por si detecto error en comienzo_foreach*/
			...
			if (!(symbolTable.get(val_peek(3).sval).getVariableType().equals(symbolTable.get(val_peek(1).sval).getVariableType()))) {
				msgStack.addMsg("Error: Tipos distintos en l铆nea: " + lex.getNewLineCounter());
				hasError = true;
				nodo = new ST_Common("ERROR", null, null);
			} else {
...
>>>>>>> 0fcca1b... varios
					...
					boolean flag = false;
					String subIndexName = ((SyntacticTree) val_peek(2)).getElem();
					if (subIndexName.contains("[")) {
						int subIndexNameStart = 0;
						while (subIndexName.charAt(subIndexNameStart) != '[')
							subIndexNameStart++;
						subIndexNameStart++; /* Salta el corchete inicial*/
						subIndexName = subIndexName.substring(subIndexNameStart + 1, subIndexName.length() - 1);
						if (symbolTable.containsKey(subIndexName)) /* Si no est谩 en la TS es porque era una constante*/
							flag = (symbolTable.get(subIndexName).isPointer() && ((SyntacticTree) val_peek(0)).getType().equals(ElementoTS.INT));
					}
...
{ 
						...
						stackOfLines.push(lex.getNewLineCounter()); 
						if(!symbolTable.get(val_peek(0).sval).isDeclared()){
							msgStack.addMsg("Error: Variable no declarada en l铆nea: " + lex.getNewLineCounter());
							hasError = true;
							...
							if (symbolTable.get(val_peek(0).sval).foreachControlState() && symbolTable.get(val_peek(0).sval).isPointer()){
								...
								((ST_Leaf)nodo).setLeftSideAssignment();
							}
						}
						yyval = nodo;
					}
...
{ 
						...
						if(!symbolTable.get(val_peek(3).sval).isDeclared()) {
							...
							if((long)symbolTable.get(val_peek(3).sval).getCSize() <= pos){
								msgStack.addMsg("Error: Posici贸n de colecci贸n superada en l铆nea: " + lex.getNewLineCounter());
								hasError = true;
								nodo  = new ST_Leaf("ERROR");
							}else{
						...
						if(!symbolTable.get(val_peek(3).sval).isDeclared() || !symbolTable.get(val_peek(1).sval).isDeclared()) {
							...
							if (symbolTable.get(val_peek(1).sval).getIdentifierClass().equals(ElementoTS.VAR)) {
								if (symbolTable.get(val_peek(1).sval).foreachControlState()) {
									...
									nodo = new ST_LeafCollection("_" + val_peek(3).sval + "[" + "_" + val_peek(1).sval + "]", nodoAux);
								} else {
								...
								msgStack.addMsg("Error: El sub铆ndice es distinto a una variable o constante en l铆nea: " + lex.getNewLineCounter());
								hasError = true;
								nodo  = new ST_Leaf("ERROR");
							}
						}
...
>>>>>>> d8fb1c1... arreglado la extracci贸n de valor del indice dentro del foreach
{
				...
				if(!symbolTable.get(val_peek(0).sval).isDeclared()){
					...
					if (symbolTable.get(val_peek(0).sval).foreachControlState()){
						...
						nodoHoja.setAlmacenamiento(nodoHoja.getElem());
						ST_Convertion nodoAux = new ST_Convertion(ST_Convertion.PTOV, nodoHoja);
						nodo = new ST_LeafCollection("_" + val_peek(0).sval, nodoAux);
					} else
						nodo = new ST_Leaf("_" + val_peek(0).sval);
					nodo.setType(symbolTable.get(val_peek(0).sval).getVariableType());
					nodo.setAlmacenamiento(nodo.getElem());
...
>>>>>>> d8fb1c1... arreglado la extracci贸n de valor del indice dentro del foreach
{
					...
					if(!symbolTable.get(val_peek(3).sval).isDeclared()){
						...
						if(symbolTable.get(val_peek(3).sval).getCSize() <= pos) {
							msgStack.addMsg("Error: Posicion de coleccion superada en l铆nea: " + lex.getNewLineCounter());
							hasError = true;
							nodo = new ST_Leaf("ERROR");
						} else
							nodo = new ST_Leaf("_" + val_peek(3).sval + "[" + val_peek(1).sval + "]");
