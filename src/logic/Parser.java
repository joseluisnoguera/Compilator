...
{ 
				...
				if (expectedVarType != "" && tipovar == ElementoTS.INT && expectedVarType == ElementoTS.LONG){
					 msgStack.addMsg("Error: incompatibilidad de tipos en la declaraci贸n de la l铆nea: " + lex.getNewLineCounter());
					 hasError = true;
				}
				expectedVarType = "";
			}
...
case 10:
//#line 50 "gramatica.y"
{ msgStack.addMsg("Error de sint谩xis en el bloque declarativo cerca de l铆nea " + lex.getNewLineCounter()); }
break;
case 11:
//#line 53 "gramatica.y"
...
{ 
		...
		if (hasError) /*Por si detecto error en comienzo_foreach por variable no declarada*/
			...
			if (!(symbolTable.get(val_peek(3).sval).getVariableType() == symbolTable.get(val_peek(1).sval).getVariableType())) {
				msgStack.addMsg("Error: Tipos distintos en el foreach de la l铆nea: " + lex.getNewLineCounter());
				hasError = true;
				nodo = new ST_Common("ERROR", null, null);
			} else {
